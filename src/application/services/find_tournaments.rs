...
use crate::domain::TournamentStage;
...
pub(in crate::application) fn find_tournaments<Repository: QueryTournaments>(
    ...
    let summaries = tournaments.iter().filter(
        |tournament|
            request.min_table_count.is_none_or(|min_table_count| tournament.table_count() >= min_table_count as usize) &&
            request.max_table_count.is_none_or(|max_table_count| tournament.table_count() <= max_table_count as usize)
    ).map(|tournament| get_tournament_summary(tournament, account_id)).collect();
    Ok(summaries)
}


pub(in crate::application) fn get_tournament_summary(tournament: &Tournament, account_id: Uuid) -> TournamentSummary {
    // TODO: make this more elegant without duplicating/exposing domain value TournamentStage
    let stage = match tournament.stage() {
        TournamentStage::WaitingForPlayers => "WaitingForPlayers",
        TournamentStage::Running => "Running",
        TournamentStage::Finished => "Finished",
    };
    TournamentSummary {
